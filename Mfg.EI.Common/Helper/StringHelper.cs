...
namespace Mfg.EI.Common
{
    public abstract class StringHelper
    {
        ...
        /// <returns></returns>
        public static string IntConvertGrade(string GradeID)
        {
            var strgrade = string.Empty;
            switch (GradeID)
            {
                case "1":
                    strgrade = "x1";
        ...
        #region 阿拉伯数字转中文  012345679   零一二三四五六七八九
        /// <summary>
        /// 阿拉伯数字转中文  012345679   零一二三四五六七八九
        /// </summary>
        /// <param name="num"></param>
        /// <returns></returns>
        public static string WholeNumberToChinese(int num)
        {
            StringBuilder sbChinese = new StringBuilder();

            string[] strDigital = new string[] { "零", "一", "二", "三", "四", "五", "六", "七", "八", "九" };//数字
            string[] strUnit = new string[] { "", "十", "百", "千", "万", "十", "百", "千", "亿", "十", "百", "千", "万", "十", "百" };//单位
            ...
            if (num < 10)
            {
                Console.WriteLine(strDigital[num]);
            ...
            {
                int n = Convert.ToInt32(strNum[i].ToString());

                if (!sbChinese.ToString().EndsWith("零") || strDigital[n] != "零")
                {
                    sbChinese.Append(strDigital[n]);
                }

                if (!sbChinese.ToString().EndsWith("零"))
                {
                    sbChinese.Append(strUnit[numLength - i - 1]);
                }
            }

            return sbChinese.ToString().TrimEnd('零');
        }
        #endregion
